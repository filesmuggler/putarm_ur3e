...
class BottleGrab(object):
  ...
  def callback(self,data):
    ...
    #goal_position = self.object_pose
    #goal_position.pose.position.y -= 0.1

    # current_pose = self.move_group.get_current_pose().pose
    # current_position = current_pose.position

    #self.move_group.set_pose_target(goal_position)
    ...
    new_pose.orientation.x = self.object_pose.pose.orientation.x
    new_pose.orientation.y = self.object_pose.pose.orientation.y
    new_pose.orientation.z = self.object_pose.pose.orientation.z
    new_pose.orientation.w = self.object_pose.pose.orientation.w
    ...
    current_pose = self.move_group.get_current_pose().pose
    all_close(waypoints[-1], current_pose, 0.01)
        
    input("zakoncz to ctrl+c")
